...
RETIRED_SOURCE_NAMES = {"\uc5d0\uc2a4\uc81c\uc774 \uad6c\ub0b4\uc2dd\ub2f9"}
...
def save_collection_sources(static_sources: list[dict], dynamic_sources: list[dict]) -> None:
    merged = {
        ...
        if entry.get("name") and entry.get("name") not in RETIRED_SOURCE_NAMES
    }
...
def main() -> None:
    ...
    HINTS_PATH.write_text(json.dumps({"sources": []}, ensure_ascii=False, indent=2), encoding="utf-8")
